...
{
    #include "export/boost/asio/result.hpp"
}
...
// This is a utility OpenSSL certification to enable boost::asio::ssl::stream[server].
// Generated without signatures. You should never use it in any other spaces.
...
namespace asioexec
{
    ...
    constexpr std::string_view key_file =
        "-----BEGIN PRIVATE KEY-----\n"
    ...
    constexpr std::string_view dh_file =
        "-----BEGIN DH PARAMETERS-----\n"
    ...
    struct tls_context_initializer
    {
        tls_context_initializer()
        {
    ...
    inline tls_context_initializer _ = tls_context_initializer();

    std::formattable<char> auto make_formattable ( auto&& args )
    {
