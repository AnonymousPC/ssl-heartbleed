...
{
    return operation_state<std::decay_t<decltype(recv)>>(std::move(self), std::forward<decltype(recv)>(recv));
}
...
class sender<types...>::async_starter_base
{
    ...
    public: // Interface
        virtual void start ( std::function<void(types...)> ) = abstract; // std::function<void(types...)> refers to boost::asio::completion_token.
};
    ...
    extends public async_starter_base
{
    ...
    public: // Interface
        void start ( std::function<void(types...)> ) override; // std::function<void(types...)> refers to boost::asio::completion_token.
};
...
template < class... types >
template < class func_type, class... arg_types >
void sender<types...>::async_starter<func_type,arg_types...>::start ( std::function<void(types...)> completion_token )
{
